...
class BCD {
  ...
  constructor(number: bigint | Uint8Array) {
    ...
    numBuffer.push(isNegative ? BCD.MINUS : BCD.PLUS);

    this.buffer = new Uint8Array(Math.ceil(numBuffer.length / 2));

    numBuffer.forEach((num, index) => {
      const bcdIndex = Math.ceil(index / 2);

      if (index % 2 === 0) {
        this.buffer[bcdIndex] = this.buffer[bcdIndex] | num;
      } else {
        this.buffer[bcdIndex] = (this.buffer[bcdIndex] << 4) | num;
      }
    });
  }
...
{
  equal(new BCD(11n).abs().valueOf(), new Uint8Array([]));
}
